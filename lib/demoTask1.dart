...
class demoTask extends StatefulWidget{
  const demoTask({Key? key, required this.userID}) : super(key: key);
  final userID;
  @override
  _demoTask createState() => _demoTask(userID);
}
class _demoTask extends State<demoTask>{
  _demoTask(this.userID);
  final userID;
  ...
  Future<String> startRecorder() async {
    ...
    await recorder.startRecorder(
      // codec: Codec.pcm,
        ...
        toFile: '$dirPath/Task1audioTrial1'
    );
    // recorderStart = DateTime.now();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: Text("Task 1 Trial 1 : ${userID}")
      ),
      body: Center(
        child: Column (
  ...
  generateCSV() async {
    ...
    // final String directory = (await getApplicationSupportDirectory()).path;
    final path = "$dirPath/tcask1Trial1.csv";
    ...
    //   MaterialPageRoute(
    //     builder: (_) {
    //      return Task1Trial2(userID: userID);
    //     },
    //   ),
    // );
  }
