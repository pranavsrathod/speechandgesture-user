...
class userTask3 extends StatefulWidget{
  final trialNum;

  const userTask3({Key? key, required this.userID, required this.trialNum}) : super(key: key);
  final userID;
  @override
  _Task3 createState() => _Task3(userID, trialNum);
}
class _Task3 extends State<userTask3>{
  final trialNum;

  _Task3(this.userID, this.trialNum);
  final userID;
  ...
  Future<String> startRecorder() async {
    ...
    await recorder.startRecorder(
      // codec: Codec.pcm,
        ...
        toFile: '$dirPath/Task3audioTrial$trialNum'
    );
    // recorderStart = DateTime.now();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: Text("Hi! Task 3 Trial $trialNum : ${userID}")
      ),
      body: Center(
        child: Column (
  ...
  generateCSV() async {
    ...
    // final String directory = (await getApplicationSupportDirectory()).path;
    final path = "$dirPath/task3Trial$trialNum.csv";
    ...
    if (trialNum == 10){
      ...
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) {
            return userTask3(userID: userID, trialNum: (trialNum + 1),);
          },
        ),
      );
    }
