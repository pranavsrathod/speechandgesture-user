...
import 'package:speechandgesture_user/practiceContrastIntro.dart';
...
class userTask2 extends StatefulWidget{
  ...
  const userTask2({Key? key, required this.userID, required this.trialNum}) : super(key: key);
  final userID;
  @override
  _Task2 createState() => _Task2(userID, trialNum);
}
class _Task2 extends State<userTask2>{
  final trialNum;

  _Task2(this.userID, this.trialNum);
  final userID;
  ...
  Future<String> startRecorder() async {
    ...
    await recorder.startRecorder(
      // codec: Codec.pcm,
        ...
        toFile: '$dirPath/Task2audioTrial$trialNum'
    );
    // recorderStart = DateTime.now();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: Text("Hi! Task 2 Trial $trialNum : ${userID}")
      ),
      body: Center(
        child: Column (
  ...
  generateCSV() async {
    ...
    // final String directory = (await getApplicationSupportDirectory()).path;
    final path = "$dirPath/task2Trial$trialNum.csv";
    ...
    if (trialNum == 10){
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) {
            return practiceContrastIntro(userID: userID);
          },
        ),
      );
    } else {
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) {
            return userTask2(userID: userID, trialNum: (trialNum + 1),);
          },
        ),
      );
    }
